...
fn get_powerup_visual(power_type: &PowerUpType) -> (Color, Mesh) {
    match power_type {
        PowerUpType::MultiShot => (
            Color::srgb(3.0, 5.0, 8.0),     // Bright cyan with glow
            create_star_mesh(5, 16.0, 8.0), // Doubled size: outer 16, inner 8
        ),
        PowerUpType::RapidFire => (
            Color::srgb(8.0, 3.0, 0.5), // Bright orange with glow
            create_diamond_mesh(12.0),  // Doubled size: 12
        ),
        PowerUpType::PiercingBullets => (
            Color::srgb(6.0, 0.5, 8.0), // Bright purple with glow
            create_triangle_mesh(16.0), // Doubled size: 16
        ),
        PowerUpType::ExplosiveBullets => (
            Color::srgb(8.0, 1.0, 1.0), // Bright red with glow
            create_hexagon_mesh(14.0),  // Doubled size: 14
        ),
        PowerUpType::LaserBeam => (
            Color::srgb(1.0, 8.0, 1.0), // Bright green with glow
            create_cross_mesh(16.0),    // Doubled size: 16
        ),
    }
}
        ...
        for player_transform in query.iter() {
            ...
            for i in 0..player_powerups.multi_shot_count {
                ...
                if player_powerups.has_piercing {
                    bullet_entity.insert(PiercingBullet::new(3)); // Can pierce through 3 asteroids
                }

                if player_powerups.has_explosive {
                    bullet_entity.insert(ExplosiveBullet::new(50.0, 2)); // 50 pixel radius, 2 damage
                }
            }
...
// Helper functions for creating power-up meshes
fn create_star_mesh(points: usize, outer_radius: f32, inner_radius: f32) -> Mesh {
    ...
    for i in 0..(points * 2) {
        indices.push(0u32);
        indices.push(((i + 1) % (points * 2)) as u32);
        indices.push(((i + 2) % (points * 2)) as u32);
    }
...
fn create_cross_mesh(size: f32) -> Mesh {
